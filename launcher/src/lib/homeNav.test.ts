...
const TILES = ["plex", "spotify", "files", "__getmore"];
...
describe("homeRowTarget", () => {
  it("enters the row above at its first key, not at the key above the cursor", () => {
    ...
    expect(homeRowTarget(ALL, "files", "up", yes)).toBe("run-plex");
    expect(homeRowTarget(ALL, "__getmore", "up", yes)).toBe("run-plex");
  });
  ...
  it("steps one row per press, in both directions", () => {
    ...
    expect(homeRowTarget(ALL, "run-plex", "down", yes)).toBe("plex");
  });
  ...
  it("leaves a row from its quit button the same way as from its chip", () => {
    ...
    expect(homeRowTarget(ALL, "runx-spotify", "down", yes)).toBe("plex");
  });

  it("skips a row that has nothing in it", () => {
    const rows = [HEADER, [], [], TILES];
    expect(homeRowTarget(rows, "plex", "up", yes)).toBe("home-settings");
    expect(homeRowTarget(rows, "home-power", "down", yes)).toBe("plex");
  });
  ...
  it("skips a key spatial navigation does not know, and the row when none is left", () => {
    ...
    expect(homeRowTarget(ALL, "files", "up", half)).toBe("runx-plex");
  });

  it("hands the press back at the top and the bottom", () => {
    expect(homeRowTarget(ALL, "home-settings", "up", yes)).toBeNull();
    expect(homeRowTarget(ALL, "__getmore", "down", yes)).toBeNull();
  });
  ...
  it("enters the rail at the tile it names first", () => {
    ...
    const remembered = [HEADER, WIDGETS, RUNNING, ["files", "plex", "spotify", "__getmore"]];
    expect(homeRowTarget(remembered, "run-plex", "down", yes)).toBe("files");
  });
});
