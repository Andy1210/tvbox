...
| **Files**   | local package            | Video and music from the box's own folders and from **USB sticks** (mounted when you open one, ejected from the same screen), played through `mpv`. Remembers where each film got to; audio track and subtitles switch while watching. See [docs/local-media.md](docs/local-media.md). |
...
the box keeps track of is which registry each app was installed from, so nobody
else's registry can take over an installed app by publishing a higher version
under its id. Unattended overnight updates are per source, off by default for
