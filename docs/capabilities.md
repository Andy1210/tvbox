...
| **3 - Privileged / native**     | A `service` plugin (Node in the host), a `runtime.bridge` (Node in the app's preload) or a `requires.download` daemon. Spotify (librespot), Live TV (IPTV proxy), anything host-side. | **Full host trust.**                                                                                | Curated registry - merge-review is the trust boundary. `requires.aptRepo` is never accepted. |
