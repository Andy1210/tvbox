...
| Windows Explorer                | Map network drive → the same URL. Windows **refuses Basic auth over plain HTTP by default**; either set `HKLM\SYSTEM\CurrentControlSet\Services\WebClient\Parameters\BasicAuthLevel` to 2 and restart the WebClient service, or use a client that does not care (WinSCP, Cyberduck, rclone itself). |
...
There is no TLS. This is a LAN service protected by a password; a self-signed
certificate would only add a warning to click through in every client.
