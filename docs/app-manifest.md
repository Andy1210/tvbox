...
| `disableService` | System services to disable after the apt install (when the shell supervises the daemon itself). Plain unit names only; a unit the box depends on (ssh, NetworkManager, greetd, systemd-\*, tvbox\*) is refused.                                    |
