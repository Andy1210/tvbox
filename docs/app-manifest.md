...
| `bridge`              | Renderer bridge adapter: a bare name is a built-in `shell/bridges/<name>.js`, `"./<file>.js"` is one the PACKAGE ships beside its manifest. Prefer the package form for anything shaped by one client (Plex's QWebChannel emulation is `./bridge.js` in its package) - it then updates from the registry instead of needing an OTA. |
