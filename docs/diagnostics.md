...
| `config.txt` size, the `vc4-kms-v3d` overlay, and either radio disabled in it | The same FAT truncation, on the file that carries the display driver: without it the box boots to nothing at all. It is written at runtime too - the built-in radio switch in Settings edits it - so it is not a file only provision ever touches. |
