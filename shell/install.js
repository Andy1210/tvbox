...
function validateManifest(m, src) {
  ...
  // Renderer bridge adapter: a bare name is one of the shell's own bridges/, and
  // "./file.js" is one the package ships next to its manifest. Both shapes are
  // pinned here so nothing that reaches require() can carry a path.
  const br = m.runtime && m.runtime.bridge;
  if (br !== undefined && (typeof br !== "string" || !/^([a-z0-9_-]+|\.\/[a-z0-9_-]+\.js)$/.test(br)))
    return bad("runtime.bridge must be a built-in name or ./<file>.js");
