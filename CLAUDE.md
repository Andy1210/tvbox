...
| Path                                                                                                                                                                                                                                                        | What                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [shell/main.js](shell/main.js)                                                                                                                                                                                                                              | **Electron host, and only that**: the windows, what is in front of what, and the wiring that hands every module below the shell state it cannot know by itself. Nothing loads this file - it requires electron - so a rule that lives here cannot be tested; that is what decides whether something belongs in it. [shell/main.load.test.js](shell/main.load.test.js) is the one check that reaches it, by replacing `electron` and loading the module body in a child process (see the note in Dev/deploy/verify).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| [shell/getroutes.js](shell/getroutes.js)                                                                                                                                                                                                                    | **The box's read API**: every GET route, and `guardedGet` - which of them the same-origin gate covers, because an open GET is the policy for a side-effect-free read and a page the box loads can drive one through an `<img>` tag that carries no Origin at all. Counterpart of routes.js.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| [shell/appinfo.js](shell/appinfo.js)                                                                                                                                                                                                                        | **What the shell says about an installed app**: `capsFor` (hard rule 2, fails closed), `switchValue`, `appLaunchable` - the one definition HOME and the Home Assistant source list share - and `bridgePath`, whose answer reaches `require()`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| [shell/plugins.js](shell/plugins.js)                                                                                                                                                                                                                        | **The plugin registry + loader**: the three things an unload has to reach (the plugin, its routes, its config listeners), the require-cache drop for its whole package dir, and `guardList` - a guard that names no route in its own table fails to load rather than guarding nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| [shell/playerapi.js](shell/playerapi.js)                                                                                                                                                                                                                    | **What an app's page may do to the shared mpv.** Foreground-only, with one exception: the app that OWNS what is loaded keeps driving it off screen - for SOUND only. See "Sound outlives the screen" below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| [shell/sharing.js](shell/sharing.js)                                                                                                                                                                                                                        | The wiring under fileserver/appshares/shares, plus the one thing none of them owns: the **lifecycle of the credential two boxes pair on** (minted under a placeholder id, adopted when the answer names the box, pruned at startup if it never was).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| [shell/tvcommand.js](shell/tvcommand.js) + [shell/mediapublish.js](shell/mediapublish.js)                                                                                                                                                                   | A command that arrived from outside the box, and the box as one `media_player` on the wire. The routing rules (which windows hear a transport command, which app the shell believes is SOUNDING) and the publish rules (coalescing, and a force that must survive landing inside a queued window) respectively.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [shell/notify.js](shell/notify.js) + [shell/widgets.js](shell/widgets.js) + [shell/powermenu.js](shell/powermenu.js) + [shell/remotepolicy.js](shell/remotepolicy.js) + [shell/crashlog.js](shell/crashlog.js) + [shell/bridgefifo.js](shell/bridgefifo.js) | The note over everything; the cards on HOME; the power menu and the sleep timer; where a remote web app may go and which cookies its manifest may plant; the record a crash leaves behind; and the two uinput bridges' control FIFOs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |     |
...
| [shell/diag.js](shell/diag.js)                                                                                                                                                                                                                              | **What one box says about itself so several can be watched at once** - published retained on `tvbox/<id>/diag`, read by the HA integration's diagnostic sensors ([docs/fleet-view.md](docs/fleet-view.md)). Two fields decide its shape: a ROLLBACK is only legible while `~/.tvbox/update/failed` survives, so the file's **mtime is the only timestamp there is** and it travels with the versions; and the wifi **link rate** comes from NetworkManager's own `Bitrate` over D-Bus, because `iw` is not in the platform baseline and nmcli only exposes a 0-100 quality, while `/proc/net/wireless` has the dBm. A link sits at full bars while its rate collapses, which is how that fault hides.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| [shell/health.js](shell/health.js) + [shell/canary.js](shell/canary.js) + [shell/configsnap.js](shell/configsnap.js)                                                                                                                                        | **Is the box working, staged OTA, and the last good configs.** health.js is the report at `GET /tvbox/api/health` and in the diag payload (a threadpool probe, a stuck install, OTA markers, a recent crash). canary.js decides when a follower may take a release another box vouched for on `tvbox/<id>/canary`; the vouch only moves the timing, the feed still chooses the release. configsnap.js keeps five 0600 copies of `config.json`, taken a while after the launcher first loads, listed and restored by the launcher only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| [launcher/src/settings/](launcher/src/settings/)                                                                                                                                                                                                            | **The Settings screen**: a category rail plus a stack of pages, and the two remote rules no page is allowed to forget. `SettingsPage.tsx` focuses a row on entry - waiting for one that arrives with the box's answer - or, if the page has none to press, scrolls itself with the arrows; `nav.tsx` gives Back the focus it came from and UNMOUNTS the level below (a hidden subtree stays in the spatial-nav tree, so the D-pad would reach rows nobody can see, and a pushed page's `render` closure freezes - so a pushed page owns its own state). `Rows.tsx` is the vocabulary, and a focus move animates NOTHING there on purpose: that is the press a remote sends twenty times a second. Pages in `settings/pages/`, one per drill-down.                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| [deploy/provision.sh](deploy/provision.sh)                                                                                                                                                                                                                  | **The ONE root step** (apt baseline, udev/polkit, linger, legacy migration). Carries `PROVISION_REVISION` - what a release's `system:<n>` requirement names - and prints a machine-readable verdict as its last line, because most of its failure branches only `warn` and exit 0 alone cannot mean "applied".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| [deploy/tvbox-sysupdate](deploy/tvbox-sysupdate) + [shell/sysupdate.js](shell/sysupdate.js)                                                                                                                                                                 | **How that root step reaches a box with no ssh.** The shell starts one polkit-granted unit and passes NOTHING; the applier verifies a signed feed against a key pinned in `/etc`, refuses anything but a strictly higher revision (a signature is not freshness - the box user can set the box's DNS), and runs the staged release's own provision.sh. [docs/updates-and-backup.md](docs/updates-and-backup.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| [remote/remote_input_bridge.py](remote/remote_input_bridge.py)                                                                                                                                                                                              | BT/USB remote → uinput bridge (user service `tvbox-remote`): EVIOCGRAB + per-device button remap, learn mode over a FIFO. Special remap actions emit no key: `power` = CEC TV toggle (`toggle 0` FIFO), `settings`/`appswitcher` = shell `/tvbox/api/nav`. Fire TV **app buttons** (vendor HID, no evdev key) are read from the remote's **hidraw** node (reports 0xEF vendor + 0x02 consumer; provision grants Amazon-VID hidraw) and injected as virtual keycodes (0x300/0x400 + usage, ANY button, nothing hardcoded; report 0x01 mirrors normal keys and stays ignored) into the same remap path. Both are HID **arrays** - the report lists what is down, so edges come from diffing it - and the consumer one's usages are **16 bits**, which is where a Remote Pro's customizable and headphone buttons live (0x27E/0x27F/0x280). **Panic reset:** same remapped raw button ×8 rapid (repeat-prone actions exempt) → `/tvbox/api/remote/reset`. Keymap in `config.remote.devices`; UI (button-test + reset + reassign-confirm) is Settings → Remotes & accessories ([launcher/src/components/RemoteRemap.tsx](launcher/src/components/RemoteRemap.tsx)).                                             |
| [voice/voice_satellite.py](voice/voice_satellite.py)                                                                                                                                                                                                        | **The remote's microphone as a Home Assistant `assist_satellite`** (user service `tvbox-voice`, Wyoming protocol, off until `config.voice.enabled`). The remote is not a Bluetooth audio device: hold the mic key (consumer usage **0x221**), write output report **`{0xF2,0x01}`** to the SAME hidraw node, and it streams 80-byte **Opus** frames on input report **0xF0** (CELT wideband, 20 ms -> 16 kHz mono, decoded here through libopus by ctypes). **The order is the trick** - the start command means nothing before the key press, which is what made this look impossible for a while. Sending it is a WRITE, so provision grants `0660` (not `0640`) on that node and **OTA cannot deliver the feature**. Home Assistant connects to US, so the box holds no URL or token; the ROOM comes from the area of the device in Home Assistant, exactly as for a Voice PE puck. [docs/voice-satellite.md](docs/voice-satellite.md).                                                                                                                                                                                                                                                                  |
...
| [shell/fileserver.js](shell/fileserver.js) + [shell/contentdirs.js](shell/contentdirs.js)                                                                                                                                                                   | **The box's folders over WebDAV** (`rclone serve webdav`, supervised, no root). The shared set is a directory of SYMLINKS rebuilt per change (`--copy-links`, writes included). Which folders are offered is DISCOVERED (contentdirs.js, shared with browse.js) - `~/.tvbox` user content (machinery filtered by name), `~/*`, and each installed flatpak's data dir, which is how an emulator's BIOS folder becomes reachable - so a folder a future app adds needs no code change. A password is mandatory (it binds to the LAN) and reaches rclone through the environment, never argv. [docs/file-server.md](docs/file-server.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| [shell/appshares.js](shell/appshares.js) + [shell/peers.js](shell/peers.js)                                                                                                                                                                                 | **An app's folders, across two boxes.** The folders an app DECLARES (`shares` in its manifest, anchored to the app's own root by the same helper `backup.paths` use) offered read-only to another tvbox, on a token of their own rather than the file server's password. The other box is found by sweeping the /24 for the pairing port - open only while a box is waiting to pair - and one code pairs both directions, because the asking box sends its own credentials with the request. A box PULLS, never pushes, so two rooms cannot overwrite each other's copy. Settings owns the permission (what is offered, which boxes); the ACTION is the app's, through the `shares` capability - the shell says nothing about saves. [docs/app-sharing.md](docs/app-sharing.md).                                                                                                                                                                                                                                                                                                                                                                                                                            |
| [shell/browse.js](shell/browse.js) + [shell/removable.js](shell/removable.js)                                                                                                                                                                               | **Local and USB playback's other half** (the app is the registry's `files` package): which roots there are, listing one, and mounting a stick. Two things decide the shape. A path is checked as a REAL path against `root + separator`, because a stick is someone else's filesystem and a local app shares our origin - `..`, a symlink to `/` and a sibling folder that merely shares a prefix all have to be refused. And a device is offered on `rm`/`tran`, never on hotplug: the Pi's own SD card reports `hotplug: true`, and a Pi booting from a USB SSD is removable by bus - either one would offer the disk the box runs from. Mounting is udisks2 + a polkit grant for `plugdev` (provision.sh), NOT the desktop's active-session default: Electron moves its main process into a systemd app scope, so polkit sees the shell with no session at all. Folder discovery is shared with the file server ([shell/contentdirs.js](shell/contentdirs.js)). [docs/local-media.md](docs/local-media.md).                                                                                                                                                                                              |
| [shell/shares.js](shell/shares.js)                                                                                                                                                                                                                          | **A NAS as a source** (Settings -> Network -> Network shares), mounted with rclone under `~/.tvbox/shares/<name>`. SMB and not NFS, because mounting NFS needs root. The mount is read-only, and **the VFS cache mode follows what the share holds**: `minimal` + read-ahead for films, `full` (capped, floored on free space, 30-day age) for games. Measured on a Pi 5: a random 64 kB read from a GameCube image over SMB costs 79 ms at the median and 560 ms in the worst percent against 1.1 ms locally - which is what mid-game freezing is made of, and why the choice is per share rather than one setting for both. Credentials reach rclone through its ENVIRONMENT, never argv.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| [shell/firetvir.js](shell/firetvir.js)                                                                                                                                                                                                                      | Fire TV remote IR programming (Settings → Remotes & accessories): venv+bleak deps install, test/program via `remote/firetv_remote_ir.py`, and the saved plan - which devices a remote drives, **with their codes inline**, because the keymap on the remote is write-only and programming must not depend on a fetch. What this file no longer does is DOWNLOAD a database: a brand used to cost up to 400 irdb CSVs plus ~95 Flipper files against an API that allows 60 unauthenticated requests an hour, so the box carried a concurrency pump, partial caching and a give-up counter. Now [shell/irindex.js](shell/irindex.js) reads one ~1.5 KB file per brand from the published index and this module decides what THIS box can send (its own python, not the index's word). Protocol encoders: [remote/ir_protocols.py](remote/ir_protocols.py) (irdb rows) + [remote/flipper_protocols.py](remote/flipper_protocols.py) (Flipper `parsed` blocks, ported from that firmware's own tables and checked against real captures); keymap bytes: [remote/keymap_compile.py](remote/keymap_compile.py). [docs/firetv-remote-ir.md](docs/firetv-remote-ir.md).                                             |
| [scripts/ir-index/](scripts/ir-index)                                                                                                                                                                                                                       | **The TV-code index, built in CI** (`.github/workflows/demo.yml`, published next to the launcher demo at `/tvbox/ir/`, weekly + on any change here). Two databases in - irdb and Flipper-IRDB, whose `raw` blocks are the reason it exists at all: irdb can only hold what a decoder understands, and a Samsung soundbar's own 36-bit protocol is not among them. One index plus a file per brand out. Three things it decides, all measured rather than assumed: codesets are merged by the FRAME they transmit (hashed through the box's own encoders, which is what also merges the two databases - 54 devices carry both), a capture that repeats one frame is cut back to one (263 of ~900 do, and the test is PERIODICITY because a Samsung frame has a 4.5 ms element in its middle), and `_Converted_/` is skipped because it IS irdb re-encoded. Its `_Converted_`-free curated tree is 1950 files.                                                                                                                                                                                                                                                                                                |
| [shell/remotefinder.js](shell/remotefinder.js)                                                                                                                                                                                                              | **Make a lost remote ring.** A Remote Pro has a buzzer behind a vendor GATT service: write `03 01` to start, `03 00` to stop, and nothing stops it by itself - so every start arms a timer. The characteristic is chosen **by its parent service**, never by UUID alone: the remote's DFU service exposes a characteristic with the SAME UUID, and a write there is a firmware update. Reaches BlueZ over D-Bus (`busctl`) rather than through a BLE client, because the remote is already connected as HID and two bytes to an open link need no venv - unlike the IR keymap, which needs bleak for MTU and chunked writes. Offered only for remotes that carry the service, and reachable over MQTT (`find_remote`) because the button you would press is on the thing you are looking for.                                                                                                                                                                                                                                                                                                                                                                                                               |
...
1. **No root at runtime.** Root lives ONLY in `deploy/provision.sh` (one-time
   udev/polkit/apt), the apt path of `tvbox deps`, and the applier that RUNS
   provision.sh (`deploy/tvbox-sysupdate`, started through one polkit-granted
   unit). Never add `sudo` to the
...
2. **`runtime.capabilities` is the security boundary** and fails closed
   (default `["nav"]`, `capsFor` in [shell/appinfo.js](shell/appinfo.js)). The launcher (id null) gets
   ...
   `plugin.js`, and less than it. `bridgePath` (appinfo.js) pins the file inside
...
senderId` in [shell/playerapi.js](shell/playerapi.js)). A queue has to cross to its next track
...
**The HOME card is derived, not pushed.** `soundWidget()` ([shell/widgets.js](shell/widgets.js)) builds it from the
...
`forwardCommand` ([shell/tvcommand.js](shell/tvcommand.js)) sent to the launcher and to the foreground app, and this rule is
...
- **A main.js change is not verified until the shell has restarted on a box** -
  but a load-time error is caught before that now.
  [shell/main.load.test.js](shell/main.load.test.js) loads main.js in a child
  process with `electron` replaced by a stub whose `whenReady()` never resolves,
  so the MODULE BODY runs and the bootstrap does not. That is exactly where this
  file's worst bug lived: an object built at module level out of `const`s declared
  further down, read while they were still in their temporal dead zone (`Cannot
access X before initialization`) - it passed `node --check`, eslint and every
  unit test, then killed the shell in a respawn loop. Build that kind of thing
...
- **Three independent uinput bridges feed input**, all user services: `tvbox-cec`
  ...
  the user explicitly remapped (Settings → Remotes & accessories) are rewritten. Device id
...
- **OTA still cannot install an apt package - but a SYSTEM UPDATE can.** The OTA
  half stays user-space by design. What changed is that a release may now declare
  `requires: ["system:<n>"]`, and a box with `tvbox-sysupdate` will fetch that
  release, verify its signature against a key pinned in `/etc`, and run its
  `provision.sh` as root out of a root-only directory - never the copy in
  `~/.tvbox`, which every OTA rewrites and the box user can edit. Before this,
  a new system-package dependency reached OTA-only boxes as code WITHOUT its
  dependency and there was no ssh on an end-user box to fix it (exactly how
  1.2.0's resolution picker stayed empty - its `wlr-randr` never arrived).
  Three things still hold. A release that needs a binary is usually better served
  by shipping it Kodi-style (`requires.download` / `installDownload`,
  sha256-pinned into `~/.tvbox/bin`) than by demanding a revision of the whole
