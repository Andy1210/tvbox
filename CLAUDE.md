...
| [shell/backup.js](shell/backup.js)                                                                  | Encrypted settings backup/restore (phone page: [shell/pairing/backup.js](shell/pairing/backup.js)). Carries config, manifests, per-app `storage` data, app-declared files (`backup.paths`/`backup.state`) and the **list of installed app ids**; what it cannot carry is re-acquired by reconcile.js. A restore onto a DIFFERENT box (`payload.machineId` != this one's) replays only `tvbox.`-prefixed localStorage keys: that store is shared by the launcher and every local app on one origin, so carrying it whole handed the second box an app's identity and login - two boxes became one Plex device. Same principle as identity.js, for renderer storage.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
