...
| [remote/remote_input_bridge.py](remote/remote_input_bridge.py)                             | BT/USB remote → uinput bridge (user service `tvbox-remote`): EVIOCGRAB + per-device button remap, learn mode over a FIFO. Special remap actions emit no key: `power` = CEC TV toggle (`toggle 0` into the CEC FIFO, state-resolved by the CEC bridge), `settings` = shell `/tvbox/api/nav`. Keymap in `config.remote.devices`; UI incl. button-test mode is Settings → Peripherals ([launcher/src/components/RemoteRemap.tsx](launcher/src/components/RemoteRemap.tsx)). |
