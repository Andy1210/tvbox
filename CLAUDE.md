...
| [shell/appshares.js](shell/appshares.js) + [shell/peers.js](shell/peers.js)                           | **Saves across two boxes.** The folders an app DECLARES (`shares` in its manifest, anchored to the app's own root by the same helper `backup.paths` use) offered read-only to another tvbox, on a token of their own rather than the file server's password. The other box is found by sweeping the /24 for the pairing port - open only while a box is waiting to pair - and a box PULLS, never pushes, so two rooms cannot overwrite each other's save. [docs/saves-sharing.md](docs/saves-sharing.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
