...
| [shell/shares.js](shell/shares.js)                                                                    | **A NAS as a source** (Settings -> Network -> Network shares), mounted with rclone under `~/.tvbox/shares/<name>`. SMB and not NFS, because mounting NFS needs root. Two things differ from the RetroArch package's share, which mounts the same way: the VFS cache is `minimal` rather than `full` (rclone's `full` downloads the whole file in the background - right for a disc image an emulator seeks in, wrong for a 60 GB film), and the mount is read-only. Credentials reach rclone through its ENVIRONMENT, never argv.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
